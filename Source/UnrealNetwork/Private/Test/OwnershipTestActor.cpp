...
AOwnershipTestActor::AOwnershipTestActor()
{
	...
	OverlapColision = CreateDefaultSubobject<USphereComponent>(TEXT("OverlapColision"));
	OverlapColision->SetupAttachment(RootComponent);
	OverlapColision->SetSphereRadius(400.0f);
...
void AOwnershipTestActor::Tick(float DeltaTime)
{
	...
	DrawDebugSphere(GetWorld(), GetActorLocation(), OverlapColision->GetScaledSphereRadius(), 32, FColor::Yellow);

	if (HasAuthority())
	{
		AActor* NextOwner = nullptr;
		float MinDistance = OverlapColision->GetScaledSphereRadius() * OverlapColision->GetScaledSphereRadius();
		TArray<AActor*> Actors;
		UGameplayStatics::GetAllActorsOfClass(GetWorld(), ARollCharacter::StaticClass(), Actors);
...
void AOwnershipTestActor::OnOverlapped(AActor* OverlappedActor, AActor* OtherActor)
{
	if (HasAuthority())
	{
		...
		{
			SetOwner(OtherActor); FString OwnerName = GetOwner() ? GetOwner()->GetName() : TEXT("No Owner");
			UE_LOG(LogTemp, Log, TEXT("Onwer : % s"), *OwnerName);
		}
	}
}
